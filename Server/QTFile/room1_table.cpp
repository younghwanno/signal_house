...
room1_table::room1_table(QSqlQuery *query)
{
    this->query = query;
    this->table = NULL;
}

void room1_table::create_table_inDB()
{
    query->exec("create table if not exists room1_sensor(date datetime not null primary key,"
                " temp int not null, humi int not null, co int, state int not null)");

    query->exec("create table if not exists room1_act(date datetime not null primary key,"
                " R int not null, G int not null, B int not null, LED_ON_OFF int not null, window int not null, door int not null)");

    query->exec("create or replace view room1_view as select a.date, a.temp, a.humi,"
                " a.co, a.state, b.R, b.G, b.B, b.LED_ON_OFF, b.window, b.door from room1_sensor a, room1_act b where a.date = b.date");
}


bool room1_table::insert_view(room1 * room)
{
...
bool room1_table::insert_sensor_table(QDateTime date, int temp, int humi, int co, int state)
{
    bool val = false;

    query->prepare("insert into room1_sensor(date, temp, humi, co, state)"
                   " VALUES(?, ?, ?, ?, ?)");
...
bool room1_table::insert_act_table(QDateTime date, int R, int G, int B, int LED_ON_OFF, int window, int door)
{
    bool val = false;

    query->prepare("insert into room1_act(date, R, G, B, LED_ON_OFF, window, door)"
                   " VALUES(?, ?, ?, ?, ?, ?, ?)");
...
QVector<room1 *> *room1_table::get_records()
{
    QVector<room1 *> *list = new QVector<room1 *>;
    query->prepare("select * from room1_view");
    query->exec();

    while(query->next())
    {
        room1 * temp = new room1();
...
QWidget* room1_table::show_table()
{
    if(this->table == NULL)
    {
        ...
        this->table->setWindowTitle("room1_table");
...
void room1_table::button_slot()
{
    QString text;

    if(comboBox->currentIndex() == 0){ // 전체검색
        text = QString("select * from room1_view");
    }else if(comboBox->currentIndex() == 1){ // 시간검색
        text = QString("select * from room1_view where date like '%%1%'").arg(lineEdit->text());
    }else if(comboBox->currentIndex() == 2){ // 온도검색
        text = QString("select * from room1_view where temp like '%%1%'").arg(lineEdit->text());
    }else if(comboBox->currentIndex() == 3){ // 습도검색
        text = QString("select * from room1_view where humi like '%%1%'").arg(lineEdit->text());
    }else if(comboBox->currentIndex() == 4){ // Co검색
        text = QString("select * from room1_view where CO like '%%1%'").arg(lineEdit->text());
        ...
        text = QString("select * from room1_view where R like '%%1%'").arg(lineEdit->text());
    }else if(comboBox->currentIndex() == 7){ // G검색
        text = QString("select * from room1_view where G like '%%1%'").arg(lineEdit->text());
    }else if(comboBox->currentIndex() == 8){ // B검색
        text = QString("select * from room1_view where B like '%%1%'").arg(lineEdit->text());
    }else if(comboBox->currentIndex() == 9){ // LED_ON_OFF검색
        text = QString("select * from room1_view where LED_ON_OFF like '%%1%'").arg(lineEdit->text());
    }else if(comboBox->currentIndex() == 10){ // window검색
        text = QString("select * from room1_view where window like '%%1%'").arg(lineEdit->text());
    }else if(comboBox->currentIndex() == 11){ // door색
        text = QString("select * from room1_view where door like '%%1%'").arg(lineEdit->text());
    }
...
QPushButton* room1_table::get_pushbutton()
{
